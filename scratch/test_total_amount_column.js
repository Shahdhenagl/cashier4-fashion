...
async function testOldColumn() {
  console.log("Testing 'total_amount' column in purchase_invoices...");
  
  const { error } = await supabase.from('purchase_invoices').select('total_amount').limit(1);
  
  if (error) {
    console.log("❌ Error selecting 'total_amount':", error.message);
  } else {
